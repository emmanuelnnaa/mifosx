...
public class LoanAssembler {
    ...
    private Loan assembleApplication(final JsonElement element, final Long clientId, final Long groupId, final AppUser currentUser) {
        ...
        final Collection<CreditCheckData> loanProductCreditCheckDataList = this.creditCheckReadPlatformService
                .retrieveLoanProductCreditChecks(productId);
        
        if (loanProductCreditCheckDataList != null && !loanProductCreditCheckDataList.isEmpty()) {
            for (CreditCheckData creditCheckData : loanProductCreditCheckDataList) {
                final CreditCheck creditCheck = this.creditCheckRepository.findOne(creditCheckData.getId());
                final LoanCreditCheck loanCreditCheck = LoanCreditCheck.instance(creditCheck);
                
                if (loanCreditCheck != null && loanCreditCheck.isActive()) {
