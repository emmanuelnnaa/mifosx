...
public class LoanCreditChecksApiResource {
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String updateLoanCreditCheck(@PathParam("loanId") final Long loanId, @PathParam("loanCreditCheckId") final Long loanCreditCheckId, 
            @QueryParam("command") final String command, final String apiRequestBodyAsJson) {
        CommandWrapper commandWrapper = null;
        
        if (compareIgnoreCase(command, "deactivate")) {
            commandWrapper = new CommandWrapperBuilder().deactivateCreditCheck(LoanApiConstants.LOAN_CREDIT_CHECK_ENTITY_NAME, loanId, loanCreditCheckId)
                    .withJson(apiRequestBodyAsJson).build();
        }
        
        else {
            throw new UnrecognizedQueryParamException("command", command, new Object[] { "deactivate" });
        }
        
        final CommandProcessingResult commandProcessingResult = this.commandsSourceWritePlatformService.logCommandSource(commandWrapper);
        
        return this.loanCreditCheckToApiJsonSerializer.serialize(commandProcessingResult);
